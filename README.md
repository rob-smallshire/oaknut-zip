...
invocation will fetch the required Python packages (`click` and `xattr`) into
an isolated cache; subsequent runs start instantly.
...
```
$ ./nutzip.py extract [OPTIONS] ZIPFILE_PATH
...
Options:
  ...
  --meta-format [acorn|pibridge|xattr|none]
                                  Metadata format (default: acorn).
  --no-nfs                        Do not decode NFS filename encoding.
  --owner INTEGER                 Econet owner ID for pibridge/xattr formats
                                  (default: 0 = SYST).
```
...
The extra field layout (all values little-endian):
...
|-------|------------------|--------------------------------------|
| `F08` | Transient command | Executed by the CLI, discarded after |
| `F09` | Transient command | Variant used by some utilities       |
| `F0E` | MOS command       | Machine Operating System utility     |
| `F1B` | Module            | Relocatable module                   |
| `FDD` | BASIC stored      | Tokenised BBC BASIC program          |
| `FFB` | BASIC text        | Textual BBC BASIC program            |
...
The test suite (128 tests) covers all parsing, formatting, and extraction logic,
