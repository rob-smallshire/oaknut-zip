...
nutzip uses a consistent `<mechanism>-<flavour>` naming scheme for format
...
```
./nutzip.py extract --meta-format filename-riscos NetUtils.zip
```
...
```
./nutzip.py extract --meta-format filename-mos NetUtils.zip
```
...
nutzip also supports two additional forms that encode full load and exec
